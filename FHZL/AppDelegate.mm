...
-(void)setShare{
    ...
    config.QQAppId = @"1106944142";
    config.QQAppKey = @"wKD3GelV7PL9Chog";
    config.WeChatAppId = WEIXINID;   //图拍 这里需要修改  还有info.plist
    config.WeChatAppSecret = @"fd0787292665291a97df36d7f548df73";
...
-(void)setRootLoginVC{
    self.homeNavi = nil;
    self.homeNavi = [[UINavigationController   alloc]initWithRootViewController:[loginVC new]];
    //默认开启系统右划返回
    self.homeNavi.interactivePopGestureRecognizer.enabled = NO;
    self.window.rootViewController = self.homeNavi;
