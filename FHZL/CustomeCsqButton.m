...
-(void)layoutSubviews{
    ...
    _ImageView.frame = CGRectMake((width - imageWidth)/2, 0, imageWidth, imageWidth);
    _TitleLabel.frame = CGRectMake(0, _ImageView.frame.size.height + _ImageView.frame.origin.y, width, titleHigth);
    _NumberLabel.frame = CGRectMake(imageWidth - numberLabelWidth/2, 0, numberLabelWidth, numberLabelWidth);
}
...
-(void)afterLayoutRefreshViewWithNormalImageStr:(NSString*)normalImageStr seleImageStr:(NSString *)seleImageStr   higligthImageStr:(NSString *)higligthImageStr titleStr:(NSString *)titleStr numberStr:(NSString *)numberStr ClickBlock:(void(^)(NSInteger numberIntData))success{
    //圆角矩形1.png
        CGFloat higth = self.frame.size.height;
        CGFloat width = self.frame.size.width;
        CGFloat imageHeight = (higth )*2/3;
    
        if ([normalImageStr isEqualToString:@"圆角矩形1.png"]) {
            imageHeight = (higth )*2/4;
        }
        CGFloat imageWidth = (higth )*2/3;
        CGFloat titleHigth = (higth )/3;
        CGFloat numberLabelWidth = 20;
        ...
        _ImageView = [[UIImageView alloc]initWithFrame:CGRectMake((width - imageWidth)/2, 0, imageWidth, imageHeight)];
    
        if ([normalImageStr isEqualToString:@"圆角矩形1.png"]) {
            _ImageView.frame = CGRectMake((width - imageWidth)/2, higth/6 - 5, imageWidth, imageHeight);
        }
    
        _ImageView.image = [UIImage imageNamed:normalImageStr];
        [self addSubview:_ImageView];
        
        _TitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, (higth )*2/3, width, titleHigth)];
        _TitleLabel.text = titleStr;
        _TitleLabel.textColor = [UIColor blackColor];
        _TitleLabel.textAlignment = NSTextAlignmentCenter;
        _TitleLabel.font = [UIFont systemFontOfSize:14];
