...
//            isFirst = NO;
        ...
        getTimer(timer,20,AllCarRequestNoUiutil)
    }
    isFirst = NO;
}
...
//                     model.macId = GetData(carDict, @"macId");
...
-(void)refreshAllCarView{
    ...
    if (CarScrView != nil) {
        for (int i = 0; i < carAllArray.count; i ++) {
            ...
            carView.macIdLable.text = model.macName; //设备名称
            
            CGFloat widthF = autoWidthFrame(model.macName,16,13).size.width + 15;
            carView.nameWidthConstraint.constant = widthF >= 160?160:widthF;
            
            
            carView.speedLabel.text = [NSString stringWithFormat:@"%ldkm/h",[model.speed integerValue]];
            carView.LocationTypeLabel.text = [NSString stringWithFormat:@"%@%%",model.battery];;
            carView.batteryLabel.text = [NSString stringWithFormat:@"%@",[model.bvalid intValue] ==1?@"GPS":[model.bvalid intValue] ==2?@"LBS":@"未定位"];//设备定位方式
            
            carView.gpsTimeLabel.text = [UserManager getDateDisplayString:[model.gpsTime longLongValue]];
            carView.gsmTimeLabel.text = [UserManager getDateDisplayString:[model.gsmTime longLongValue]];
            carView.statusLabel.text = [NSString stringWithFormat:@"%@",[model.status intValue] == 0?@"点火":@"熄火"];
            carView.stopLabel.text = [model.online intValue] ==1?@"在线":@"离线";
            
            carView.carDoorLabel.text = [NSString stringWithFormat:@"%@",[model.door intValue] == 0?@"关闭":@"打开"];//
            
            
            float width = kScreenWidth - 20   ;
            if ([model.macType isEqualToString:DR_30]) {
                carView.carDoorLabel.hidden = YES;
                carView.carDoorImage.hidden = YES;
                carView.GpsImageLeadingContast.constant = width/4 - 40;
                carView.batteryImageLeadingContast.constant = width/4 - 10;
            }
            if ([model.macType isEqualToString:GT31]) {
                carView.carDoorLabel.hidden = YES;
                carView.carDoorImage.hidden = YES;
                carView.statusImageV.hidden = YES;
                carView.statusLabel.hidden = YES;
                carView.GpsImageLeadingContast.constant = - width/4 + 7.5;
                carView.batteryImageLeadingContast.constant = 6;
            }
...
//        carView.buttonView.hidden = carScrHeigth==Clock?YES:NO;
        ...
        carView.carDoorLabel.text = [NSString stringWithFormat:@"%@",[model.door intValue] == 0?@"关闭":@"打开"];//
        if ([model.macType isEqualToString:DR_30] || [model.macType isEqualToString:GT51]) {
            carView.carDoorLabel.hidden = YES;
            carView.carDoorImage.hidden = YES;
            carView.GpsImageLeadingContast.constant = width/4 - 40;
            carView.batteryImageLeadingContast.constant = width/4 - 10;
        ...
        DISPATCH_ON_MAIN_THREAD((^{
            NSArray *buttonArray = @[@"追踪",@"轨迹",@"围栏",@"指令",@"警报",@"详情"];
            NSArray *imageArray = @[@"列表_4_N.png",@"列表_6_N.png",@"列表_7_N.png",@"列表_8_N.png",@"列表_9_N.png",@"列表_10_N.png",];
            NSArray *higthLigthImageArray = @[@"列表_4_P.png",@"列表_6_P.png",@"列表_7_P.png",@"列表_8_P.png",@"列表_9_P.png",@"列表_10_P.png",];
...
- (IBAction)changeCarCount:(id)sender {
    ...
    if (!_changeCarButton.selected) {
        [UIUtil showToast:@"进入单车页面" inView:self.view];
        [self addOneAnnotation];
        CarScrView.scrollEnabled = NO;
    }else{
        [UIUtil showToast:@"进入多车页面" inView:self.view];
        [self addAllAnnotation];
        CarScrView.scrollEnabled = YES;
    }
...
-(void)getAddressClick:(UIButton *)but{
    ...
    if (kStringIsEmpty(model.addressStr)) {
        [but setTitle:@"正在获取地理位置" forState:UIControlStateNormal];
    ...
    [ZZXDataService HFZLRequest:@"track/get-tracks" httpMethod:@"POST" params1:dic   file:nil success:^(id data)
     {
         if ([data[@"code"]integerValue] == 0) {
             ...
             if (!kArrayIsEmpty(userArray)) {
                 ...
                 [USERMANAGER.GT10CarArray addObjectsFromArray:carAllArray];
                 [USERMANAGER save];
                 [self refreshAllCarView];//refreshAllCarView reSetAllCarView
             }else{
//                 [UIUtil showToast:@"未绑定设备" inView:self.view];
                 [USERMANAGER.GT10CarArray removeAllObjects];
                 [USERMANAGER save];
                 [self reSetAllCarView];//refreshAllCarView reSetAllCarView
             }
         }else{
             switch ([data[@"code"]integerValue]) {
                 case 200002:
