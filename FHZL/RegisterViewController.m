...
- (IBAction)getPhoneCode:(id)sender {
    if (_GetCodeButton.selected) {
        ...
        [UIUtil showToast:@"手机号码不能为空" inView:self.view];
    }
    else if (![AppData valiMobile:_PhoneTF.text]) {
        [UIUtil showToast:@"手机号码不符规范" inView:self.view];
    }
    else{
...
- (IBAction)FirstNextStep:(id)sender {
    if (kStringIsEmpty(_PhoneTF.text)) {
        ...
        [UIUtil showToast:@"验证码不能为空" inView:self.view];
    }
    else{
...
- (IBAction)TwoNextStep:(id)sender {
    if (kStringIsEmpty(_firstPassWordTF.text) || kStringIsEmpty(_TowPassWordTF.text)) {
        [UIUtil showToast:@"密码不能为空" inView:self.view];
    }
...
//    }
        ...
        [UIUtil showToast:@"昵称不能为空" inView:self.view];
    }
    else if (_acceptButton.selected) {
...
//                })
            ...
            [UIUtil showToast:@"您还未安装微信" inView:self.view];
        }
    }
    else{
...
-(void)WEIXINIMAGEFirst{
    [UIUtil showProgressHUD:@"正在注册..." inView:self.view];
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)codeRequest{
    [UIUtil showProgressHUD:@"获取短信验证码" inView:self.view]; //登录..
    ...
    [ZZXDataService HFZLRequest:@"sms/verify-code" httpMethod:@"POST" params1:dic   file:nil success:^(id data)
     {
         ...
         if ([data[@"code"]integerValue] == 0) {
             ...
             switch ([data[@"code"]integerValue]) {
                 case 203001:
                 {
                     [UIUtil showToast:@"一分钟内不能重复发送短信" inView:self.view];
                 }
                     break;
                 case 203002:
                 {
                     [UIUtil showToast:@"一小时内短信发送过多" inView:self.view];
                 }
                     break;
                 case 203003:
                 {
                     [UIUtil showToast:@"今日短信已全部用完" inView:self.view];
                 }
                     break;
                 case 101014:
                 {
                     [UIUtil showToast:@"手机号已被注册" inView:self.view];
                 }
                     break;
                 default:
                     [UIUtil showToast:@"发送失败" inView:self.view];
                     break;
             }
         }
     } fail:^(NSError *error)
...
-(void)RegisterRequest{
    [UIUtil showProgressHUD:@"正在注册..." inView:self.view];
    NSString *headStr = _acceptButton.selected?USERMANAGER.user.headPicPath:@"";
    ...
    [ZZXDataService HFZLRequest:@"user/register" httpMethod:@"POST" params1:dic   file:nil success:^(id data)
     {
         if ([data[@"code"]integerValue] == 0) {
             ...
             switch ([data[@"code"]integerValue]) {
                 case 101013:
                     {
                         [UIUtil showToast:@"用户协议未统一" inView:self.view];
                     }
                     break;
                 case 101014:
                 {
                 ...
                 case 101031:
                 {
                     [UIUtil showToast:@"设备号已被绑定" inView:self.view];
                 }
                     break;
                 case 101012:
                 {
                     [UIUtil showToast:@"验证码错误" inView:self.view];
                 }
                     break;
                     
                 default:
                     [UIUtil showToast:@"注册失败" inView:self.view];
                     break;
             }
