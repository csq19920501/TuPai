...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if (indexPath.section == 1)
    {
        ...
        if (indexPath.row == 1)
        {
            cell.titleLabel.text = @"消息中心";//好友分享
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            nameAlertView  = [[UIAlertView alloc] initWithTitle:nil message:@"输入您想要的名称" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        ...
        if (indexPath.row == 4) {
            ...
            if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"weixin://"]]) {
                ...
                if (headImageIsEmpty(USERMANAGER.user.headPicPath) ) {
                    ...
                    showNewAlert(nil,@"是否取消绑定微信头像",nil,(^(UIAlertAction *action){
                        [UIUtil showProgressHUD:@"正在解绑" inView:self.view];
...
//                                     USERMANAGER.user.upPicPath = GetData();
                                 ...
                                 [UIUtil showToast:@"解绑失败" inView:self.view];
                             }
                         } fail:^(NSError *error)
                         {
...
//    }
    ...
    if (indexPath.section == 2) {
        showNewAlert(nil,@"是否确定退出",nil,^(UIAlertAction *action){
            [UIUtil showProgressHUD:@"正在退出..." inView:self.view];
    ...
    if ([lang isEqualToString:@"zh-Hans"]) { // 简体中文输入，包括简体拼音，健体五笔，简体手写
        ...
        if (!position) {
            if (toBeString.length > kMaxLength) {
                ...
                [UIUtil showToast:@"长度不能超过16位" inView:self.view];
    ...
    if (alertView.tag == 52) {
        if (buttonIndex == 1) {
            if (kStringIsEmpty(nameTF.text)) {
                [UIUtil showToast:L(@"名称不能为空") inView:self.view];
            }else{
                [UIUtil showProgressHUD:@"请稍候..." inView:self.view];
                NSDictionary *dic = @{@"userId":CsqStringIsEmpty(USERMANAGER.user.userID),
