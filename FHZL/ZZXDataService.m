             ...
             fail:(void(^)(NSError *error))fail
{
    ...
    NSLog(@"secretString = %@",secretString);
              ...
              fail:(void(^)(NSError *error))fail
{
    ...
    //1.拼接地址
    NSString *requestURL = [NSString stringWithFormat:@"%@/%@/%@",urlStr,ServiceBuild, string];
            ...
            [manager POST:encodeURL parameters:mutableDict success:^(NSURLSessionDataTask * _Nullable task, id responseObject)
             {
                 if (success)
                 {
//                     id responseObject = [NSJSONSerialization JSONObjectWithData:responseObject
