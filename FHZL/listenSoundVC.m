...
- (IBAction)recordClick:(id)sender {
    ...
    if (!isSeleOne) {
        [UIUtil showToast:@"请选择单个设备" inView:self.view];
        return;
    }
    if ([USERMANAGER.seleCar.macType isEqualToString:GT31] || [USERMANAGER.seleCar.macType isEqualToString:GT51]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"该设备类型不支持录音功能" message:nil preferredStyle:  UIAlertControllerStyleAlert];
...
- (NSArray *)getMenuModelsArray {
    ...
    FFDropDownMenuModel *menuModel0 = [FFDropDownMenuModel ff_DropDownMenuModelWithMenuItemTitle:@"全部设备" menuItemIconName:nil  menuBlock:^{
        isSeleOne = NO;
...
-(void)NoGT10{
    if (kArrayIsEmpty(USERMANAGER.GT10CarArray)) {
        [UIUtil showProgressHUD:@"请绑定设备" inView:self.view];
        return;
    }
}
...
-(void)changeEmptyView{
    if (alarmDataArray.count == 0) {
        [_myTableView addEmptyViewWithImageName:@"" title:@"当前设备没有录音记录"];
...
-(void)playRecord:(NSString*)wavPath{
    if (kStringIsEmpty(wavPath)) {
        [UIUtil showToast:@"播放路径出错" inView:self.view];
        [self stopRecord];
        return;
    }
    ...
    [ZZXDataService HFZLRequest:@"instruction/send" httpMethod:@"POST" params1:dic   file:nil success:^(id data)
     {
         ...
         if ([data[@"code"]integerValue] == 101054) {
             if (intCount <= mostNunber) {
                 ...
                 [UIUtil showToast:@"连接超时" inView:self.view];
                 switCH.on = !switCH.on;
             }
         }else if([data[@"code"]integerValue] == 0){
             [UIUtil showToast:@"发送指令成功" inView:self.view];
             switch (tag) {
                 case 0:
                    {
                        if (TimeTimer == nil) {
