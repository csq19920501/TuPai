...
- (void)viewDidLoad {
    ...
    NSAttributedString *attStr = [[NSAttributedString alloc] initWithString:@"账号" attributes:attrs]; // 初始化富文本占位字符串
    _accountTF.attributedPlaceholder = attStr;
    NSAttributedString *attStr2 = [[NSAttributedString alloc] initWithString:@"密码" attributes:attrs]; // 初始化富文本占位字符串
...
- (IBAction)loginClick:(id)sender {
    ...
    if (kStringIsEmpty(_accountTF.text)) {
        [UIUtil showToast:@"账号不能为空" inView:self.view];
    }
    else if (![AppData valiMobile:_accountTF.text]) {
        [UIUtil showToast:@"账号不符合规范" inView:self.view];
    }
...
-(void)LoginRequest{
    [UIUtil showProgressHUD:@"正在登录..." inView:self.view];
    ...
    [ZZXDataService HFZLRequest:@"user/login" httpMethod:@"POST" params1:dic   file:nil success:^(id data)
     {
         if ([data[@"code"]integerValue] == 0) {
             [UIUtil showToast:@"登录成功" inView:self.view];
...
//             [self.navigationController pushViewController:[HomeVC new] animated:YES];
             ...
             switch ([data[@"code"]integerValue]) {
                 ...
                 case 101021:
                 {
                     [UIUtil showToast:@"账号不存在或者密码错误" inView:self.view];
                 }
                     break;
                 default:
