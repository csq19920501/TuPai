...
- (void)csqActionSheet:(takePhoto)tpBlock ppBlock:(pickPhoto)pkBlock csqBlock:(csqBlock)csqBlock{
    UIAlertController *alertCtl = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *takePhoto = [UIAlertAction actionWithTitle:@"拍照上传" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    UIAlertAction *csqPhoto = [UIAlertAction actionWithTitle:@"系统头像" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (csqBlock) {
            csqBlock();
        }
    }];
